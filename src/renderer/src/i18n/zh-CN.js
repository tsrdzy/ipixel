export default {
  common: {
    upload: '上传',
    delete: '删除',
    cancel: '取消',
    confirm: '确认',
    ok: '确定',
    close: '关闭',
    edit: '编辑',
    add: '添加',
    name: '名称',
    size: '大小',
    date: '日期',
    type: '类型',
    tags: '标签',
    format: '格式',
    all: '全部',
    none: '无',
    select: '选择',
    selected: '已选',
    unselected: '未选择',
    search: '搜索',
    sort: '排序',
    previewSize: '预览大小',
    displayFields: '显示字段',
    selectAll: '全选',
    preview: '预览',
    detail: '详细信息',
    openDetail: '打开详细信息',
    refresh: '刷新',
    backToTop: '回到顶部',
    batch: '批量',
    single: '单个',
    cover: '封面',
    animation: '动画',
    dimensions: '尺寸',
    uploadTime: '上传时间',
    fileSize: '文件大小',
    fileType: '文件类型',
    noData: '暂无数据',
    noMatch: '没有匹配的内容',
    empty: '未命名',
    back: '返回',
    export: '导出',
    exporting: '导出中',
    save: '保存',
    saving: '保存中',
    loading: '加载中',
    processing: '处理中',
    files: '文件',
    failed: '失败',
    update: '已更新',
    main: '主',
    mtl: '材质',
    texture: '贴图',
    aux: '辅',
    success: '成功',
    reset: '重置',
    loadFailed: '加载失败',
    uploadSuccess: '上传成功',
    dropHere: '松开鼠标以上传',
    invalidFormat: '格式不正确，仅支持 {formats}',
    dragUploadProgress: '正在上传 {current}/{total}',
    theme: {
      dark: '暗色',
      light: '亮色',
      toggleDark: '切换到暗色',
      toggleLight: '切换到亮色'
    },
    language: '语言',
    languages: {
      chinese: '中文',
      english: 'English',
      spanish: 'Español',
      french: 'Français',
      german: 'Deutsch',
      japanese: '日本語',
      russian: 'Русский'
    },
    appTitle: 'iPixel - 素材资源库'
  },
  sidebar: {
    model: '模型资源库',
    image: '图片资源库',
    audio: '音频资源库',
    font: '字体资源库',
    tools: '工具箱',
    comingSoon: '功能开发中，敬请期待...'
  },
  menu: {
    tutorial: '使用教程',
    more: '更多',
    tutorialTitle: '使用教程',
    tutorial1: '1. 创建资源库：点击「创建资源库」按钮，选择保存位置并命名。',
    tutorial2: '2. 上传模型：点击上传按钮，支持单个上传（可编辑详情）和批量上传（自动生成封面）。',
    tutorial3: '3. 选择模型：单击选择单个模型，按住 Ctrl/Cmd 多选，按住 Shift 连续选择，鼠标拖拽框选。',
    tutorial4: '4. 管理标签：在模型详情或批量操作中添加/删除标签，便于分类搜索。',
    tutorial5: '5. 预览模型：双击模型查看详细信息和3D预览，调整视角后保存封面。',
    feedback: '意见反馈',
    checkUpdate: '检查更新',
    checkingUpdate: '正在检查更新...',
    updateAvailable: '发现新版本',
    newVersion: '发现新版本 {version}，是否前往下载？',
    download: '下载',
    latestVersion: '当前已是最新版本!',
    newVersionAvailable: '有新版本!',
    checkUpdateFailed: '检查更新失败，请检查网络连接',
    dontRemind: '不再提醒',
    about: '关于 iModel',
    version: '版本',
    settings: '设置',
    shortcuts: '快捷键',
    shortcutSelectAll: '全选',
    shortcutDelete: '删除选中',
    shortcutExport: '导出选中',
    shortcutNavigate: '导航选择',
    shortcutArrowUp: '上移',
    shortcutArrowDown: '下移',
    shortcutArrowLeft: '左移',
    shortcutArrowRight: '右移',
    operationLogs: '操作日志',
    aboutDesc: '一个简洁高效的3D模型资源管理工具，支持GLB、GLTF、OBJ、STL等多种格式。',
    license: '开源协议：MIT License'
  },
  settings: {
    general: '常规',
    appearance: '外观',
    language: '语言',
    theme: '主题',
    skin: '皮肤',
    dark: '暗色',
    light: '亮色',
    showInTitlebar: '在顶部显示',
    version: '版本',
    support: '支持我们',
    supportTitle: '爱发电支持',
    supportDesc: '如果喜欢这个项目，欢迎支持一下',
    supportBtn: '去支持',
    shortcuts: '快捷键',
    pressKey: '按下按键...',
    resetShortcutsConfirm: '确定要重置所有快捷键吗？'
  },
  init: {
    title: '资源库管理',
    selectLibrary: '选择资源库',
    create: '创建资源库',
    open: '打开资源库',
    recent: '最近使用',
    empty: '还没有资源库',
    createDesc: '创建一个新的资源库来管理您的3D模型',
    openDesc: '打开已有的资源库',
    libraryName: '资源库名称',
    libraryPath: '资源库路径',
    browse: '浏览',
    creating: '正在创建...',
    opening: '正在打开...',
    confirmDelete: '确定要删除这个资源库吗？此操作不可撤销。',
    rename: '重命名',
    deleteLibrary: '删除资源库',
    switchLibrary: '切换资源库',
    closeLibrary: '关闭资源库'
  },
  home: {
    searchPlaceholder: '搜索模型名称...',
    tagsPlaceholder: '标签: 全部',
    formatsPlaceholder: '格式: 全部',
    selectAll: '全选',
    selectInverse: '反选',
    clearSelection: '取消选择',
    addTag: '添加标签',
    batchDelete: '批量删除',
    batchAddTag: '批量添加标签',
    singleUpload: '单个上传',
    batchUpload: '批量上传',
    noModel: '还没有模型，点击「上传模型」添加第一个',
    noMatch: '没有匹配的模型',
    coverPlaceholder: '无预览图',
    animationCount: '{count} 个动画',
    moreTags: '+{count}',
    confirmBatchDelete: '确定要删除选中的 {count} 个模型吗？此操作不可撤销。',
    enterTag: '请输入标签',
    tagAdded: '标签已添加',
    tagsAdded: '{count} 个标签已添加',
    deletionSuccess: '删除成功',
    deletionFailed: '删除失败'
  },
  image: {
    duplicate: '图片已存在',
    confirmDelete: '确认删除这张图片？',
    items: '张图片',
    width: '宽度',
    colorFilter: '颜色',
    tolerance: '容差',
    dimensions: '尺寸',
    dominantColor: '主色',
    secondaryColor: '辅色',
    detail: '图片详情',
    empty: '暂无图片，点击右上角上传',
    searchPlaceholder: '搜索图片名称',
    namePlaceholder: '请输入图片名称',
    nameRequired: '请输入图片名称',
    fileInfo: '文件信息',
    colors: {
      black: '黑色',
      white: '白色',
      gray: '灰色',
      darkgray: '深灰',
      lightgray: '浅灰',
      red: '红色',
      darkred: '深红',
      coral: '珊瑚红',
      salmon: '三文鱼色',
      orange: '橙色',
      gold: '金色',
      yellow: '黄色',
      khaki: '卡其色',
      green: '绿色',
      darkgreen: '深绿',
      lime: '酸橙绿',
      olive: '橄榄绿',
      teal: '青色',
      turquoise: '绿松石色',
      cyan: '天青色',
      blue: '蓝色',
      darkblue: '深蓝',
      navy: '藏青',
      indigo: '靛蓝',
      purple: '紫色',
      violet: '紫罗兰',
      magenta: '品红',
      plum: '梅红',
      pink: '粉色',
      brown: '棕色',
      silver: '银色',
      orchid: '兰花紫'
    }
  },
  audio: {
    duplicate: '音频已存在',
    confirmDelete: '确认删除这个音频？',
    items: '个音频',
    duration: '时长',
    sampleRate: '采样率',
    channels: '声道数',
    detail: '音频详情',
    empty: '暂无音频，点击右上角上传',
    searchPlaceholder: '搜索音频名称',
    namePlaceholder: '请输入音频名称',
    nameRequired: '请输入音频名称',
    fileInfo: '文件信息'
  },
  font: {
    duplicate: '字体已存在',
    confirmDelete: '确认删除这个字体？',
    items: '个字体',
    fontFamily: '字体系列',
    fontWeight: '字重',
    fontStyle: '字体样式',
    detail: '字体详情',
    empty: '暂无字体，点击右上角上传',
    searchPlaceholder: '搜索字体名称',
    namePlaceholder: '请输入字体名称',
    nameRequired: '请输入字体名称',
    fileInfo: '文件信息',
    sampleTextTop: '素材资源库'
  },
  tools: {
    image: '图片工具',
    video: '视频工具',
    audio: '音频工具',
    font: '字体工具',
    model: '模型工具',
    other: '其他工具',
    categoryDesc: '选择一个工具使用',
    noTools: '该分类下暂无工具',
    comingSoon: '即将推出',
    lastUpdate: '最后更新',
    imageSplit: '图片分割',
    imageSplitDesc: '将图片分割成多个部分',
    imageMerge: '图片合并',
    imageMergeDesc: '将多张图片合并为一张',
    imageCompress: '图片压缩',
    imageCompressDesc: '压缩图片大小，支持多种压缩质量',
    imageFormat: '图片格式转换',
    imageFormatDesc: '将图片转换为PNG、JPG、BMP、GIF等格式',
    icoGenerator: 'ICO生成器',
    icoGeneratorDesc: '将图片转换为ICO格式图标',
    imagePixel: '图片像素化',
    imagePixelDesc: '将图片转换为像素风格，支持1-10级像素化程度',
    gifGenerator: 'GIF生成器',
    gifGeneratorDesc: '将多张图片合成GIF动图',
    audioSpeed: '音频变速',
    audioSpeedDesc: '调整音频播放速度，支持导出和导入到素材库',
    audioVideoSplit: '音视频分离',
    audioVideoSplitDesc: '从视频中分离音频，支持导出和导入到素材库',
    gifSettings: 'GIF设置',
    gifPreview: 'GIF预览',
    width: '宽度',
    height: '高度',
    frameInterval: '帧间隔',
    quality: '画质',
    generate: '生成',
    selectImages: '选择图片',
    imageCount: '图片数量',
    outputSize: '输出尺寸',
    mergeMode: '排列方式',
    horizontal: '横向排列',
    vertical: '纵向排列',
    grid: '宫格排列',
    gridCols: '宫格列数',
    cols: '列',
    rowGap: '上下行间距',
    colGap: '左右列间距',
    merge: '合并',
    clear: '清空',
    noImages: '请先选择图片',
    selectImage: '选择图片',
    clickSelect: '点击选择图片',
    selectAudio: '选择音频',
    selectVideo: '选择视频',
    speedAdjustment: '速度调整',
    playbackSpeed: '播放速度',
    originalDuration: '原时长',
    newDuration: '新时长',
    process: '处理',
    processSuccess: '处理成功',
    extractAudio: '提取音频',
    extractSuccess: '提取成功',
    audioVideoSplit: '音视频分离',
    exportSuccess: '导出成功',
    processResult: '处理结果',
    speed: '速度',
    duration: '时长',
    extractResult: '分离结果',
    fileName: '文件名',
    pixelSettings: '像素设置',
    pixelLevel: '像素化程度',
    pixelTips: '数值越大，像素化效果越明显',
    operation: '操作',
    manualSplit: '手动分割',
    autoSplit: '自动分割',
    singleWidth: '单图宽度',
    singleHeight: '单图高度',
    margin: '外边距',
    horizontalGap: '左右间距',
    verticalGap: '上下间距',
    bgColor: '背景颜色',
    split: '分割',
    splitResult: '分割结果',
    mergeResult: '合并结果',
    exportLocal: '导出到本地',
    exportResources: '导出到资源库',
    noResult: '暂无结果',
    download: '下载'
  },
  upload: {
    title: '上传模型',
    editTitle: '编辑模型',
    editModel: '编辑模型',
    uploadModel: '上传模型',
    exportTitle: '导出模型到指定位置',
    selectModel: '点击或拖拽 3D 模型文件',
    formats: '支持 GLB / GLTF / OBJ / STL / JSON / FBX',
    saveCover: '保存封面',
    resetView: '重置视角',
    coverSaved: '封面已保存',
    coverPreview: '封面预览',
    coverImage: '封面',
    coverPlaceholder: '调整视角后点击「保存封面」',
    tip: '拖拽旋转 · 滚轮缩放 · 右键平移，调整到满意视角后点击「保存封面」',
    description: '简介',
    descriptionPlaceholder: '描述模型用途、特点等',
    modelName: '请填写模型名称',
    fileInfo: '文件信息',
    fileName: '文件名',
    format: '格式',
    fileSize: '文件大小',
    dimensions: '尺寸',
    uploadTime: '上传时间',
    relatedFiles: '关联文件',
    delete: '确认删除',
    deleteConfirm: '删除后无法恢复，确定删除该模型吗？',
    confirmDuplicate: '发现重复模型「{name}」，是否覆盖？\n覆盖将删除旧模型及其封面、标签等信息。',
    overwrite: '覆盖',
    skip: '跳过',
    exportCount: '已导出 {count} 个文件到：{dir}',
    loadModel: '加载模型',
    addTag: '添加标签',
    removeTag: '移除标签'
  }
}
